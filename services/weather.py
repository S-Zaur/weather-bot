...
def extract_hourly_data(response: openmeteo_sdk.WeatherApiResponse) -> dict:
    hourly_temperature = [
        round(response.Hourly().Variables(0).Values(i), 1)
        for i in range(response.Hourly().Variables(0).ValuesLength())
    ]
    hourly_wind_speed = [
        round(response.Hourly().Variables(1).Values(i), 1)
        for i in range(response.Hourly().Variables(1).ValuesLength())
    ]
    hourly_wind_direction = [
        get_wind_direction(response.Hourly().Variables(2).Values(i))
        for i in range(response.Hourly().Variables(2).ValuesLength())
    ]
    hourly_wind_gusts = [
        round(response.Hourly().Variables(3).Values(i))
        for i in range(response.Hourly().Variables(3).ValuesLength())
    ]
    rain = [
        round(response.Hourly().Variables(4).Values(i), 1)
        for i in range(response.Hourly().Variables(4).ValuesLength())
    ]
    prob = [
        round(response.Hourly().Variables(5).Values(i), 1)
        for i in range(response.Hourly().Variables(5).ValuesLength())
    ]
    uv_index = [
        round(response.Hourly().Variables(6).Values(i), 1)
        for i in range(response.Hourly().Variables(6).ValuesLength())
    ]
    return {
...
def extract_minutely_data(response: openmeteo_sdk.WeatherApiResponse) -> dict:
    rain_minutely = [
        round(response.Minutely15().Variables(0).Values(i), 1)
        for i in range(response.Minutely15().Variables(0).ValuesLength())
    ]
    precipitation_minutely = [
...
def extract_current_data(response: openmeteo_sdk.WeatherApiResponse) -> dict:
    weather_desc, weather_emoji = get_weather_desc(
...
def interpret_current_weather(weather: dict) -> str:
    ...
    rain = (
        "без осадков"
        if weather["rain"] == 0
        else f"дождь интенсивностью {weather['rain']} мм/ч"
    )
...
def interpret_hourly_weather(temp, wind_speed, wind_direction, wind_gusts, uv_index):
    ...
    uv_index_sum = f"УФ индекс до {max(uv_index)}"
...
def interpret_rain_data(daily_data: dict, hourly_data: dict) -> str:
    ...
    for i in ranges:
        s = slice(*i)
        rains.append(
            f"с {i[0]} до {i[1]} часов с максимальной интенсивностью {max(rain[s])} мм/ч и вероятностью {max(prob[s])}%"
        )
    return (
        f"Сегодня ожидаются дожд{letters[0]} в период{letters[1]}: {', '.join(rains)}"
    )
